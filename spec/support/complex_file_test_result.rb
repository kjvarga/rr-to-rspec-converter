...
describe SendPetitionStarterCommentedNotifications do
  ...
  def dont_allow_email_notification
    ...
    expect(Resque).not_to receive(:enqueue_in).with(anything, PetitionStarterCommentedEmailWorker)
  end

  def dont_allow_onsite_notification
    allow(Fluent::Logger).to receive(:post)
    expect(Fluent::Logger).not_to receive(:post).with(anything)
  end
  ...
  describe '#send_notifications' do
    ...
    context 'when sending an email notification' do
      ...
      context 'when the total recipient count exceeds the SFMC limit' do
        before do
          ...
          allow_any_instance_of(Petition).to receive(:total_signature_count).and_return(2)
        end
    ...
    context 'when sending an onsite notification' do
      ...
      it 'sends a fluent event' do
        expect(Fluent::Logger).to receive(:post).with('monorail.petition_starter.petition_comment.created.onsite', petition_id: petition.id, comment_id: petition_comment.id, created_at: petition_comment.created_at)
        described_class.new(comment: petition_comment, commentable_entity: petition).send_notifications
      end
      ...
      context 'when the feature is not enabled' do
        ...
        it 'does not send a fluent event' do
          allow(Fluent::Logger).to receive(:post)
          expect(Fluent::Logger).not_to receive(:post).with('monorail.petition_starter.petition_comment.created.onsite', anything)
          described_class.new(comment: petition_comment, commentable_entity: petition).send_notifications
        end
      end
    end
